...
Dijkstra 的证明适用于连接成环并**由中央守护进程激活的复合（非素数）数量的处理器**。回想一下，中央守护进程一次激活一个处理器，然后激活的处理器读取其邻居的状态并相应地改变其状态。处理器 $P_i$ 可以在配置 $c$ 中执行关键部分，当且仅当 $P_i$ 在 $c$ 之后由中央守护进程首先激活时改变其状态。因此，每次公平执行自稳定互斥算法都必须有一个后缀，其中每个配置中只有一个处理器可以改变状态。
